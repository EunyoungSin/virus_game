...
https://virus-game-blond.vercel.app/ 에서 확인 해 보실 수 있습니다.
...
더 이상 조건으로 쓰지 않습니다 — 다른 탭을 보고 있어도 이 게임 탭이 닫히지 않은 한 계속
카운트됩니다). 탭을 닫거나 다른 라우트로 이동해 화면이 언마운트될 때만 전송이 멈춥니다(이 경우는
불이익 없음). 브라우저가 백그라운드 탭의 타이머를 스로틀링해 정확히 30초 간격이 안 지켜질 수
...
아키타입이라고 감염에서 배제되지 않습니다. 감염자로 확정된 방문자의 `infection_stage`도
`day_index` 기준 가중치로 뽑습니다("먼저 감염된 사람일수록 증상이 심하다"는 논리) —
1일차는 LATE(증상 뚜렷) 비중이 70%로 높아 문답만으로도 비교적 쉽게 알아챌 수 있고, 4일차는
EARLY(증상 옅음) 비중이 40%까지 올라가 판별이 어려워집니다(`VisitorGenerationService.
INFECTION_STAGE_WEIGHTS_BY_DAY`, 4일 구조 기준 고정표라 `game.days`가 4가 아니면 1~4로
clamp). 증상 발현 확률표(`SYMPTOM_PROBABILITY_RANGE`)와 조합되어 LATE 비중이 높은 1일차는
자동으로 증상 발현률도 높아지고 EARLY 비중이 높은 4일차는 증상 발현률도 낮아집니다. 여기에
1일차는 "불안" 비중을 높게, 마지막 날에 가까워질수록 "침착·뻔뻔" 비중을 높이는 성격 커브까지
겹쳐 후반부 난이도를 한 번 더 올립니다.
...
없어 네트워크 지연이 생기면 요청 스레드가 무기한 대기할 수 있었습니다. 참고로 `generativelanguage
.googleapis.com`은 Vertex AI와 달리 리전을 선택할 수 없는 글로벌 엔드포인트라, 백엔드/DB를 어느
리전에 두든 Gemini 호출 자체의 지연 시간은 줄어들지 않습니다(Render 무료 플랜의 CPU 스로틀링이
체감 속도에 더 큰 영향을 줍니다) — DB만 백엔드와 같은 리전에 두면 그쪽 왕복은 빨라집니다.
...
단, 이 화면(entry-context=FROM_TITLE)에서는 응답에 `gameStatus`/`endingType`이 있어도
"종료됨 · BAD" 같은 엔딩 뱃지를 표시하지 않습니다 — 일반 슬롯과 똑같이 보이며 그대로 로드됩니다
(엔딩 정보는 엔딩 기록 보관소에서만 확인). 게임 내 "불러오기"/"저장" 화면(entry-context=
FROM_IN_GAME)에서는 이 뱃지가 계속 표시됩니다. 엔딩 기록 보관소(`ArchiveView.vue`, `/archive`)는
여전히 완전히 분리된 읽기 전용 화면입니다.
...
검사키트 결과는 판정 도장과 톤을 의도적으로 분리합니다(`src/components/TestKitSlip.vue`) —
도장이 "내가 방금 내린 결정"을 쾅 찍는 임팩트 모션이라면, 검사키트는 "이미 정해져 있던 사실"이
조용히 드러나는 것이라 화면 흔들림이나 블러 임팩트를 전혀 쓰지 않습니다. 사용 시 자원
카운터(`KIT n`)가 미세하게 한 번 흔들리고, 방문자 카드 옆에서 작은 종이 슬립이 슬라이드인되어
결과(양성=stamp-red, 음성=quarantine 텍스트)를 보여준 뒤 몇 초 후 슬라이드아웃됩니다. 슬립이
사라진 뒤에도 방문자 카드에는 "검사 완료: 양성/음성" 태그가 영구적으로 남아(다음 방문자로
넘어가면 초기화) 이 방문자를 검사했다는 사실 자체를 잊지 않게 합니다.
