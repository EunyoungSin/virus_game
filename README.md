...
AI 기반 감염자 판별 텍스트 게임. 바이러스 봉쇄 도시의 출입관리소 직원이 되어 방문자와 대화를 통해 감염 여부를
추리하고 출입을 허가/거부하는 1인 텍스트 어드벤처.
...
게임 목록에서 슬롯 목록(`SlotPicker.vue`, `GameListView.vue`)으로 바뀌면서, ✕ 버튼은 이제
`DeleteSaveSlot`(슬롯 하나만 비움, `DeleteConfirmModal.vue` 재사용 없이 `SlotPicker.vue` 내장
2단계 인라인 확인)만 호출합니다. 게임 자체를 통째로 지우는 화면은 아직 없습니다 — 이번 스펙 문서가
"사건 이어하기 = 슬롯 목록"으로 화면을 재정의하면서 생긴 간극으로, 필요하면 별도로 "내 사건 전체
관리" 같은 화면을 추가해야 합니다.
...
**"사건 이어하기"는 게임 목록이 아니라 저장 슬롯 목록입니다.** `GameListView.vue`(`/games`)는
`GET /api/users/{userId}/saves`로 슬롯 5개를 조회해 `SlotPicker.vue`(mode=LOAD, scope=GLOBAL)로
보여줍니다. 점유된 슬롯을 클릭하면 `SimpleConfirmModal`("이 시점으로 되돌립니다")로 확인받은 뒤
그 슬롯이 가리키는 게임으로 `load`하고 곧장 플레이 화면(`game-play`)으로 들어갑니다 — 어느 게임의
슬롯인지는 슬롯 자체가 이미 알고 있으므로, 로드에 성공한 게임의 `gameId`로 라우팅합니다(선택한
슬롯이 다른 게임을 가리켜도 정상 동작). ✕ 버튼은 `SlotPicker.vue` 내장 2단계 인라인 확인(한 번
누르면 "확인?" 상태로 3초 대기, 그 안에 다시 누르면 실행)으로 `DELETE
...
플레이 화면의 "저장" 버튼은 항상 `SlotPicker.vue`(mode=SAVE, scope=GLOBAL)를 엽니다. 빈 슬롯을
클릭하면 확인 없이 바로 저장되고, 이미 기록이 있는 슬롯을 클릭하면 그 슬롯이 **현재 플레이 중인
게임과 같은 게임**을 가리키는지에 따라 확인 방식이 갈립니다 — 같은 게임의 다른 시점이면 가벼운
`SimpleConfirmModal`("이 시점을 덮어씁니다"), 다른 게임이면 `SlotOverwriteConfirmModal.vue`
("슬롯 충돌" 워터마크의 무거운 경고, 대상 게임 번호·일차·종료 여부 표시)를 씁니다.

"← 목록으로" 버튼은 즉시 이동하지 않습니다. 먼저 `GET /api/users/{userId}/saves`를 조회해 **현재
플레이 중인 게임을 가리키는 슬롯이 있는지** 확인합니다:
- 하나도 없으면 `ExitToListConfirmModal.vue`("저장 이후 진행한 판정과 대화는 모두 사라집니다")를
  보여주고, 확인 시 API 호출 없이 곧장 목록으로 이동합니다(되돌릴 저장본 자체가 없으므로 현재
  진행 상태는 그대로 살아있는 채로 남지만, 슬롯이 없어 다시 접근할 방법은 없습니다).
- 하나 이상 있으면 `SlotPicker.vue`(mode=LOAD, scope=CURRENT_GAME, 이 게임을 가리키는 슬롯만
  필터링)를 보여주고, 슬롯 선택 후 `SimpleConfirmModal` 확인을 거쳐 `load`한 뒤 목록으로 이동합니다.
"취소"를 누르면 두 경로 모두 아무 API도 호출하지 않고 현재 화면에 그대로 남습니다.

일자 전환 연출("N일차 종료" 뒤 뜨는 "저장하시겠습니까?" 블로킹 게이트, 아래 참고)의 저장은 슬롯
선택 화면을 열지 않습니다 — 이미 사용자 클릭을 기다리는 블로킹 단계 안에서 또 다른 화면을 열면
흐름이 무거워지므로, 이 게임이 이미 차지한 슬롯이 있으면 그걸 자동으로 갱신하고 없으면 빈 슬롯
중 하나를 자동으로 골라 씁니다(쓸 수 있는 슬롯이 전혀 없으면 조용히 건너뜁니다 — 이 넛지 하나
때문에 하루 전환이 막혀서는 안 됩니다).
...
형광등 깜빡임은 `src/components/FlickeringLight.vue`에 있습니다. 'Special Elite'/'Noto Serif
KR'/'JetBrains Mono' 폰트를 Google Fonts에서 불러오므로 최초 로드 시 인터넷 연결이 필요합니다.
`prefers-reduced-motion`을 존중해 도장 임팩트·형광등 깜빡임 모두 정적 버전으로 대체됩니다.

하루가 끝날 때(3번째 방문자 판정 직후, 마지막 4일차는 제외 — 그 경우는 곧바로 결과 화면으로)
`src/components/DayTransitionOverlay.vue`가 재생됩니다: 정적(0.4s, 형광등도 이 순간만 멈춤) →
서류철 닫힘 와이프 + 오늘 통계 도장 찍기(1.2s) → **저장 확인(블로킹)** → 서류철 열림 + 형광등
이중 깜빡임(0.9s). "저장하시겠습니까? 예/아니오"는 애니메이션이 아니라 사용자 클릭을 기다리는
블로킹 게이트(`awaitingSaveConfirm`)입니다 — 타이머로는 절대 풀리지 않고, 오직 클릭으로만
다음 단계로 넘어갑니다. "예"를 누르면 이 게임이 이미 차지한 슬롯이 있으면 그걸 자동 갱신하고,
없으면 빈 슬롯 중 하나를 자동으로 골라 저장합니다(슬롯 선택 화면을 열지 않습니다 — 이미 블로킹
중인 흐름 안에서 또 다른 화면을 열면 무거워지므로). 애니메이션 자체(1/2/4단계)는 최초 1회(브라우저당,
`localStorage` 플래그)는 스킵 불가이고 이후부터는 우측 상단 "스킵" 버튼이 나타나지만, 3단계(저장
확인)는 스킵 대상이 아니며 클릭할 때까지 무기한 대기해도 정상입니다.
...
## 다음 단계 (MVP 우선순위)
1. ~~게스트 인증(deviceId + JWT), 회원가입/로그인/계정전환 없음~~ (완료)
2. ~~Game 생성/요약 API (개수 제한 없음, 저장 슬롯 5개가 실질적 제약)~~ (완료)
3. ~~Visitor 생성 로직 (아키타입 12종 전부 1명씩, 감염자 무작위 배정, 증상 확률 분리)~~ (완료)
4. ~~Conversation API (Gemini API 연동, provider 추상화 포함)~~ (완료)
5. ~~Decision API + Day 전환 + 감염자 통과 수/오탐 하향/신뢰 붕괴 조기종료 엔딩 계산~~ (완료)
6. ~~저장/불러오기 API (유저 전역 슬롯 5개, 완전 스냅샷 방식, game_saves)~~ (완료)
7. ~~Vue 프론트: 게스트 로그인 → 게임 목록 → 대화 → 결과 화면 연결, 저장/불러오기/검사키트 버튼~~ (완료)
8. ~~대화 이력 조회 API~~ (완료)
9. ~~검사키트 사용 API~~ (완료)
10. 배포: Render(백엔드) + Supabase/Render(DB) + Vercel(프론트)
11. (확장) 재회 모순 탐지 강조 UI, 일시정지 게임 재개 API
